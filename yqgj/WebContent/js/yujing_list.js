...
function init_layui_nav_child(mylayer, id) {
	if(cookie.get("zhuanti_count_data") == null){
		$.ajax({
			type: "GET",
			url: "http://114.55.218.124:5000/yujing_keyword",
			dataType: "json",
			success: function(data) {
				cookie.set("zhuanti_count_data",JSON.stringify(data));
				var layui_nav_child = $(".layui-side-scroll .layui-nav-tree .layui-nav-child");
				var yujing_keyword_list = data.data.yujing_keyword;
				for (var index in yujing_keyword_list) {
					var yujing = yujing_keyword_list[index];
					var dd = $("<dd><a href='javascript:;'>" + yujing.keyword + "</a></dd>");
		...
		var yujing_keyword_list = JSON.parse(cookie.get("zhuanti_count_data")).data.yujing_keyword;
		var layui_nav_child = $(".layui-side-scroll .layui-nav-tree .layui-nav-child");
		for (var index in yujing_keyword_list) {
			var yujing = yujing_keyword_list[index];
			var dd = $("<dd><a href='javascript:;'>" + yujing.keyword + "</a></dd>");
...
function get_yuqing_list(mylayer, id, cursor,is_init_page) {
	if (is_init_nav == 0 || is_init_tab == 0) {
		...
		console.log(media);
		console.log(emotion);
		var zhuanti = $(".gaoxiao_title").text();
		console.log(zhuanti);
